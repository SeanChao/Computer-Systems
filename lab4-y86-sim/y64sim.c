...
char *stat_name(stat_t e) {
    if (e < STAT_AOK || e > STAT_INS) return "Invalid Status";
    return stat_names[e];
}
...
bool_t get_byte_val(mem_t *m, long_t addr, byte_t *dest) {
    if (addr < 0 || addr >= m->len) return FALSE;
    *dest = m->data[addr];
    return TRUE;
}
...
bool_t get_long_val(mem_t *m, long_t addr, long_t *dest) {
    int i;
    long_t val;
    if (addr < 0 || addr + 8 > m->len) return FALSE;
    val = 0;
    for (i = 0; i < 8; i++) val = val | ((long_t)m->data[addr + i]) << (8 * i);
    *dest = val;
    return TRUE;
}
...
stat_t nexti(y64sim_t *sim) {
    ...
    byte_t rA = 0xF;
    byte_t rB = 0xF;
    if (icode == I_RRMOVQ || icode == I_RMMOVQ || icode == I_MRMOVQ ||
        icode == I_IRMOVQ || icode == I_ALU || icode == I_PUSHQ ||
        icode == I_POPQ) {
        byte_t reg = 0;
    ...
    switch (icode) {
        ...
        case I_ALU: /* 6:x regA:regB */
            if (ifun < 0 || ifun > 4) {
                err_print("PC = 0x%lx, Invalid instruction %.2x", sim->pc,
                          codefun);
                return STAT_INS;
            }
        ...
        case I_RET: /* 9:0 */
            ...
            valE = valB + 8;
            set_reg_val(sim->r, REG_RSP, valE);
            if(!get_long_val(sim->m, valA, &next_pc)) {
                err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc,
                          valE);
                return STAT_ADR;
            }
...
int main(int argc, char *argv[]) {
    ...
    for (step = 0; step < max_steps && e == STAT_AOK; step++) e = nexti(sim);
